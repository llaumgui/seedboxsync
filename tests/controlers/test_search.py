...
def test_seedboxsync_search_downloaded():
    ...
    with SeedboxSyncTest(argv=argv, config_dirs=SeedboxSyncTest.get_config_dirs()) as app:
        ...
        assert SeedboxSyncTest.hash_output(output) == 'd5358298b49cb0359b379000d215ec41'
    ...
    with SeedboxSyncTest(argv=argv, config_dirs=SeedboxSyncTest.get_config_dirs()) as app:
        app.run()
        data, output = app.last_rendered
        assert SeedboxSyncTest.hash_output(output) == '013c431a1bf733bfb3108d3e692ff9c8'
    ...
    with SeedboxSyncTest(argv=argv, config_dirs=SeedboxSyncTest.get_config_dirs()) as app:
        app.run()
        data, output = app.last_rendered
        assert SeedboxSyncTest.hash_output(output) == '6f8c477b46b38e1c87aa5af21b95a67c'
...
def test_seedboxsync_search_progress():
    ...
    with SeedboxSyncTest(argv=argv, config_dirs=SeedboxSyncTest.get_config_dirs()) as app:
        app.run()
        data, output = app.last_rendered
        assert SeedboxSyncTest.hash_output(output) == 'e776b53155595b6cc66bd21e6c16a547'
    ...
    with SeedboxSyncTest(argv=argv, config_dirs=SeedboxSyncTest.get_config_dirs()) as app:
        app.run()
        data, output = app.last_rendered
        assert SeedboxSyncTest.hash_output(output) == '892f588b5a6b2b45a29a40ec61a43465'
...
def test_seedboxsync_search_uploaded():
    ...
    with SeedboxSyncTest(argv=argv, config_dirs=SeedboxSyncTest.get_config_dirs()) as app:
        app.run()
        data, output = app.last_rendered
        assert SeedboxSyncTest.hash_output(output) == 'eddf5be7df1164d0e13da4df497a8ce3'
    ...
    with SeedboxSyncTest(argv=argv, config_dirs=SeedboxSyncTest.get_config_dirs()) as app:
        app.run()
        data, output = app.last_rendered
        assert SeedboxSyncTest.hash_output(output) == 'd9da6c5c513564702263eb84417c133a'
    ...
    with SeedboxSyncTest(argv=argv, config_dirs=SeedboxSyncTest.get_config_dirs()) as app:
        app.run()
        data, output = app.last_rendered
        assert SeedboxSyncTest.hash_output(output) == '674e18ee2dac8f603524d90d21131631'
