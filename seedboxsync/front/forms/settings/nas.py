...
class SettingsNasForm(FlaskForm):  # type: ignore[misc]
    """
    Form for configuring local NAS storage paths.

    Manages path definitions for local torrent watch folders and local
    ...
    local_download_path = StringField(
        _("Downloads path"),
        validators=[DataRequired()],
        render_kw={"placeholder": "/downloads", "icon": "fa-folder", "help": _("Path where files are downloaded.")},
    )
