...
from seedboxsync.core import current_app, utils
...
def __exclude_by_pattern(filepath: str | PathLike[str]) -> bool:
    ...
    """
    pattern = current_app.seedboxsync_config.get("seedbox_exclude_syncing", "")
...
def __get_file(filepath: str | PathLike[str], only_store: bool) -> None:
    ...
    """
    filepath = fspath(filepath)
    local_filepath = Path(current_app.seedboxsync_config.get("local_download_path", "")).expanduser().resolve() / filepath
    part_suffix = current_app.seedboxsync_config.get("seedbox_part_suffix", "")
    ...
    current_app.logger.debug(f'Download: "{filepath}" to "{local_path}"')

    try:
        seedbox_size = current_app.sync.stat(filepath).st_size
        if seedbox_size == 0:
            current_app.logger.warning(f'Empty file: "{filepath}" ({seedbox_size!s})')

        download = Download.create(path=filepath, seedbox_size=seedbox_size)
        download.save()

        if not only_store:
            current_app.logger.info(f'Downloading "{filepath}"')
            progress_callback = DownloadProgress(download)
            current_app.sync.get(filepath, local_filepath_part, progress_callback=progress_callback)
            local_size = Path(local_filepath_part).stat().st_size

            if local_size == 0 or local_size != seedbox_size:
                current_app.logger.error(f'Download failed: "{filepath}" ({local_size!s}/{seedbox_size!s})')

            Path(local_filepath_part).rename(local_filepath)
        else:
            current_app.logger.info(f'Marking as downloaded "{filepath}"')
        ...
        current_app.logger.error(f"Download failed: {exc!s}")
